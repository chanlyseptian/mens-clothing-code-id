...
import { getProductsSortPrice, getProductsPopular } from "../actions/cmsActions"
...
const HighlightProduct = () => {
    ...
    const {
        actionPrice, statusPrice, dataPrice,
        actionPopular, statusPopular, dataPopular,
    ...
    useEffect(() => {
        if (actionPopular === "GET_PRODUCTS_POPULAR" && statusPopular === "dataPopular") {
                            ...
                            {dataPrice.data && dataPrice.data.map((data) => (
                                <SwiperSlide>
                                    ...
                                    <div className='w-[220px] inline-block p-2 cursor-pointer hover:scale-105 ease-in-out duration-300'>
                                        ...
                                        />
                                        <p className="px-2 justify-between text-start text-gray-200 w-[200px] truncate overflow-hidden">{data.name}</p>
                ...
                >
                    <div >
                        <Swiper
                            ...
                            className="h-[420px]"
                        >
                            {dataPopular.data && dataPopular.data.map((populer) => (
                                <SwiperSlide>
                                    <div className='w-[220px] inline-block p-2 cursor-pointer hover:scale-105 ease-in-out duration-300'>
                                        <img
                                            className="w-[200px] h-[300px]"
                                        ...
                                        />
                                        <p className="px-2 justify-between text-start text-gray-200 w-[200px] overflow-ellipsis overflow-hidden">{populer.name}</p>
                                        <div className="flex justify-between px-2 ">
                                            <div>
                                                <div>
                                                    <p className="text-start text-gray-200 font-bold">
                                            ...
                                            <div className="text-sm text-gray-200">
                                                <p className="text-[12px] capitalize text-end">{populer.category}</p>
                                                <p className="font-semibold text-end">Sold: {populer.totalSold}</p>
                                            </div>
                                        </div>
                                    </div>
                                </SwiperSlide>
